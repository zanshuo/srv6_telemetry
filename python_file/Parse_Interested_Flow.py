...
class Parse_interested_flow:
    ...
    def generate_data(self):
        self.path_list=Parse_interested_flow.path_dict[self.namespace_id]
    ...
    def parse_data(self,sequence):
        ...
        for tmp_sequence in range(self.start_sequence,sequence+1):
            ...
            cursor = conn.cursor()
            cursor.execute("select src,export_timestap,export_transit_delay,export_packet_length from %s where namespace_id=%d and sequencenumber=%d and flowid=%d"%(self.table_name,self.namespace_id,tmp_sequence,self.flowid))
            ...
            if len(values) == len(self.path_list):
                ...
                if False not in flag:
                    ...
                    for serial in range(len(self.path_list)):
                        if serial == len(self.path_list)-1:
                            break
                        link_delay=self.node_data_dict[self.path_list[serial+1]]["export_timestap"] - self.node_data_dict[self.path_list[serial]]["export_timestap"]
                        - self.node_data_dict[self.path_list[serial]]["export_transit_delay"][-1]
        ...
        node_dict=dict()
        link_dict=dict()
        for x,y in self.node_data_dict:
            ...
            node_dict[Parse_interested_flow.sid_to_name[x]]["loss_rate"] = "%.3f%%"%((y["loss_packet_count"]/y["receive_packet_count"])*100)

        for x,y in self.link_delay_dict:
            link_dict[x] ="%.3fms"%((sum(y)/len(y))/1000)
        return node_dict,link_dict
if __name__ == "__main__":
    namespaceid_for_path_dict= {1:["2001::1","2002::2","2004::4","2006::6"]}