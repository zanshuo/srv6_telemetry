...
class SingleSwitchTopo(Topo):
    def __init__(self, sw_path, json_path, thrift_port, pcap_dump, **opts):
        ...
        switch1 = self.addSwitch('s1', sw_path=sw_path, json_path=json_path, thrift_port=thrift_port, cls=P4Switch,
                                 pcap_dump=pcap_dump)
        # switch2 = self.addSwitch('s2', sw_path=sw_path, json_path=json_path, thrift_port=thrift_port + 1, cls=P4Switch,
        #                          pcap_dump=pcap_dump)
        # switch3 = self.addSwitch('s3', sw_path=sw_path, json_path=json_path, thrift_port=thrift_port + 2, cls=P4Switch,
        #                          pcap_dump=pcap_dump)
        # switch4 = self.addSwitch('s4', sw_path=sw_path, json_path=json_path, thrift_port=thrift_port + 3, cls=P4Switch,
        #                          pcap_dump=pcap_dump)
...
def main():
    ...
    switch_outside=net.switches[0]
    _intf_1 = Intf(intFname, node=switch_outside, port=255)
    net.start()
